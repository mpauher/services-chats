                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    @foreach ($chats as $chat)
                        <div class="my-3 max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700"
                            ...
                            <div class="p-5 ">
                                <a
                                    href={{ route('chat.show', ['service_id' => $chat->service_id, 'id' => $chat->chat_id]) }}>
                                    <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">
                                        {{ $chat->service_title }}<h5>
                                </a>
                                <p class="mb-3 font-normal text-gray-700 dark:text-gray-400">
                                    {{ $chat->guest_user_name }}
                                </p>
                            </div>
                        </div>
                    @endforeach
                    @foreach ($guest_chats as $guest_chat)
                    <div class="my-3 max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700"
                        style="margin-bottom: 10px;">
                        <a href=#>
                            <img class="rounded-t-lg " src="no-img.jpg" alt="" />
                        ...
                        <div class="p-5 ">
                            <a
                                href={{ route('chat.show', ['service_id' => $guest_chat->service_id, 'id' => $guest_chat->chat_id]) }}>
                                <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">
                                    {{ $guest_chat->service_title }}<h5>
                            </a>
                            <p class="mb-3 font-normal text-gray-700 dark:text-gray-400">
                                {{ $guest_chat->guest_user_name }}
                            </p>
                        </div>
                    </div>
                @endforeach
